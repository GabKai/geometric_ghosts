...
const static int TEMPLATE_UP = 1000;
const static int LEVELUP_CLICK_DELAY = 2.0f;
static float levelUpTimer = 0.0f;
static bool isTemplateUp = false;
static LevelUpScreenState levelUpState;
...
void DrawUI(void)
{
    ...
    char hpText[16];
    sprintf(hpText, "%d / %d", player.hp, player.maxHp);
...
void DrawLevelUp(void)
{
    ...
    if (levelUpState.options[0] == NULL || levelUpState.options[1] == NULL)
        return;

    Vector2 mousePos = GetMousePosition();
    levelUpState.clickTimer -= GetFrameTime();
    bool canClick = (levelUpState.clickTimer <= 0.0f);

    float cardWidth = 500.0f;
    float cardHeight = 100.0f;
    float cardX = (GetScreenWidth() / 2.0f) - (cardWidth / 2.0f);

    float startY = (GetScreenHeight() / 2.0f) - cardHeight - 20.0f;
    float gap = 30.0f;

    for (int i = 0; i < 2; i++)
    {
        ShootTemplate *template = levelUpState.options[i];

        float currentCardY = startY + (i * (cardHeight + gap));
        Rectangle cardRect = {cardX, currentCardY, cardWidth, cardHeight};

        bool isHovered = CheckCollisionPointRec(mousePos, cardRect);

        Color baseColor = isHovered ? Fade(PURPLE, 0.3f) : Fade(BLACK, 0.5f);
        Color borderColor = isHovered ? GOLD : PURPLE;
        if (!canClick && isHovered)
            borderColor = DARKGRAY;

        DrawRectangleRec(cardRect, baseColor);
        DrawRectangleLinesEx(cardRect, 2.0f, borderColor);

        float iconX = cardX + 20.0f;
        float iconY = currentCardY + (cardHeight / 2.0f) - 32.0f;
        Rectangle sourceRec = {0.0f, 0.0f, (float)levelUpState.options[i]->texture.width, (float)levelUpState.options[i]->texture.height};
        Rectangle destRec = {iconX, iconY, 64.0f, 64.0f};
        Vector2 origin = {0.0f, 0.0f};
        DrawTexturePro(levelUpState.options[i]->texture, sourceRec, destRec, origin, 0.0f, canClick ? RAYWHITE : Fade(RAYWHITE, 0.4f));

        float textStartX = iconX + 64.0f + 20.0f;
        DrawText(template->name, textStartX, currentCardY + 20, 16, GOLD);
        DrawText(template->text, textStartX, currentCardY + 50, 12, RAYWHITE);

        if (canClick && isHovered && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
        {
            levelUpState.selectedOption = i;
            player.shootTemplate = *levelUpState.options[i];
            gameData->currentState = STATE_GAME;
        }
    }

    if (levelUpState.clickTimer > 0.0f)
    {
        float barHeight = 12.0f;
        float barX = (screenWidth / 2.0f) - (cardWidth / 2.0f);
        float barY = startY + (2 * (cardHeight + gap));

        float progressPercent = levelUpState.clickTimer / LEVELUP_CLICK_DELAY;
        float currentProgressWidth = cardWidth * progressPercent;
        DrawRectangleRec((Rectangle){barX, barY, cardWidth, barHeight}, DARKGRAY);
        DrawRectangleRec((Rectangle){barX, barY, currentProgressWidth, barHeight}, ORANGE);
    }
}
...
void CheckCollisions(void)
{
    ...
    if (gain.score > 0)
    {
        ...
        if (!isTemplateUp && gameData->score >= TEMPLATE_UP)
        {
            isTemplateUp = true;
            gameData->currentState = STATE_GAMEUP;
            TriggerLevelUp();
        }
    }
}
...
void TriggerLevelUp(void)
{
    while (true)
    {
        ShootTemplate *t = GetRandomShootTemplate();
        if (t == NULL || strcmp(t->name, "simple") == 0)
        {
            continue;
        }

        levelUpState.options[0] = t;
        break;
    }
    ...
    while (true)
    {
        ...
        if (strcmp(t->name, levelUpState.options[0]->name) == 0)
        {
            continue;
        }

        levelUpState.options[1] = t;
        break;
    }
    levelUpState.clickTimer = LEVELUP_CLICK_DELAY;
    levelUpState.selectedOption = -1;
}